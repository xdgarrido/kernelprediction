function [model, varargout] = traingmlvq(trainSet, trainLab, varargin)
...
p.addOptional('optimization', 'adam', @(x)any(strcmpi(x,{'sgd','adam'})));
...
p.addOptional('beta1', 0.9,@(x)(isfloat(x) && x>=0));
p.addOptional('beta2', 0.999,@(x)(isfloat(x) && x>=0));
...
if isa(p.Results.learningRatePrototypes,'function_handle')
    % with a given function specified from the user
    alphas = arrayfun(p.Results.learningRatePrototypes, 1:nb_epochs);
elseif length(p.Results.learningRatePrototypes)>3
    if length(p.Results.learningRatePrototypes)==nb_epochs
        alphas = p.Results.learningRatePrototypes;
    else
        disp('The learning rate vector for the prototypes does not fit the nb of epochs');
    ...
    % or use an decay with a start and a decay value
    if isempty(p.Results.learningRatePrototypes)
        initialization.learningRatePrototypes = [nb_features/100, 0.75, 10];
    end
    alpha_start = initialization.learningRatePrototypes(1);
    decay_rate = initialization.learningRatePrototypes(2);
    decay_steps = initialization.learningRatePrototypes(3);
    % alphas = arrayfun(@(x) alpha_start * (alpha_end/alpha_start)^(x/nb_epochs), 1:nb_epochs);
    % alphas = arrayfun(@(x) alpha_start / (1+(x-1)*alpha_end), 1:nb_epochs);

   for e=1:nb_epochs
        alphas(e) = alpha_start * decay_rate ^ floor((1. + (e-1.))/decay_steps);  
   end 
end
% compute the vector of nb_epochs learning rates epsilon for the Matrix learning
epsilons = zeros(1,nb_epochs);
...
% 	epsilons = arrayfun(p.Results.learningRateMatrix, 1:nb_epochs);
%    epsilons(MatrixStart:nb_epochs) = arrayfun(p.Results.learningRateMatrix, MatrixStart:nb_epochs);
elseif length(p.Results.learningRateMatrix)>3
    if length(p.Results.learningRateMatrix)==nb_epochs
        epsilons = p.Results.learningRateMatrix;
    else
        disp('The learning rate vector for the Matrix does not fit the nb of epochs');
    ...
    % or use an decay with a start and a decay value
    if isempty(p.Results.learningRateMatrix)
        initialization.learningRateMatrix = [nb_features/1000, 0.75, 10];
    end
    eps_start = initialization.learningRateMatrix(1);
    decay_rate = initialization.learningRateMatrix(2);
    decay_steps = initialization.learningRateMatrix(3);
    % epsilons = arrayfun(@(x) eps_start * (eps_end/eps_start)^(x/nb_epochs), 1:nb_epochs);
    % epsilons(MatrixStart:nb_epochs) = arrayfun(@(x) eps_start * (eps_end/eps_start)^((x-MatrixStart)/(nb_epochs-MatrixStart)), MatrixStart:nb_epochs);
   
   for e=1:nb_epochs
        epsilons(e) = eps_start * decay_rate ^ floor((1. + (e-1.))/decay_steps);  
   end 
end
...
%             dist = diag(dist);
            ...
            countJ = 1;
            J = sortIdx(countJ);
            while model.c_w(sortIdx(countJ)) ~= c_xi, 
                countJ = countJ+1;
                J = sortIdx(countJ);
            end
            dJ = sortDist(countJ);
            countK = 1;
            K = sortIdx(countK);
            while model.c_w(sortIdx(countK)) == c_xi, 
                countK = countK+1;
                K = sortIdx(countK);
            end
            dK = sortDist(countK);
    %         disp([J,K,dJ,dK]);
            ...
            wK = model.w(K,:);
            % prototype update
            norm_factor = (dJ + dK)^2 + 0.00000001;
            DJ = (xi-wJ);
            DK = (xi-wK);
            oo = model.omega'*model.omega;
            mu = (dJ - dK)/((dJ + dK) + 0.00000001);
            act = swish_prime(mu,1.0);
    ...
    %         dwK = (2*dJ/norm_factor)*2*model.omega'*model.omega*DK';
            model.w(J,:) = wJ + act * alphas(epoch) * (dwJ');
            model.w(K,:) = wK - act * alphas(epoch) * (dwK');
            % update matrices
            if epsilons(epoch)>0, % epoch >= MatrixStart
                ...
                model.omega = model.omega- act .* epsilons(epoch) * (f1-f2  - regularization * f3);
                % normalization
        ...
        if nout>=2,
            ...
            msg = ['trainerror =',num2str(trainError(epoch+1))];
            disp(msg);
...
%%% gradient descent variables
    ...
    mJ = zeros(nb_features,1);
    vJ = zeros(nb_features,1);
    mK = zeros(nb_features,1);
    vK = zeros(nb_features,1);
    mhJ = zeros(nb_features,1);
    vhJ = zeros(nb_features,1);
    mhK = zeros(nb_features,1);
    vhK = zeros(nb_features,1);
    mf = zeros(nb_features);
    vf = zeros(nb_features);
    mf2 = zeros(nb_features);
    vf2 = zeros(nb_features);    
    % compute the vector of nb_epochs learning rates alpha for the prototype learning
    if isa(p.Results.learningRatePrototypes,'function_handle')
        % with a given function specified from the user
        alphas = arrayfun(p.Results.learningRatePrototypes, 1:nb_epochs);
...
%             dist = diag(dist);
    ...
    %         disp([J,K,dJ,dK]);
            ...
            dwJ = act * (2*dK/norm_factor)*2*oo*DJ';
            dwK = act * (2*dJ/norm_factor)*2*oo*DK';
            mJ = beta1 * mJ + (1. - beta1) * dwJ; 
            mK = beta1 * mK + (1. - beta1) * dwK;
            vJ = beta2 * vJ + (1. - beta2) * (dwJ.^2); 
            vK = beta2 * vK + (1. - beta2) * (dwK.^2);
            beta1_power  = power(beta1,iter);
            beta2_power  = power(beta2,iter);
            mhJ = mJ ./(1. - beta1_power); 
            mhK = mK ./(1. - beta1_power); 
            vhJ = vJ ./(1. - beta2_power); 
            vhK = vK ./(1. - beta2_power);
            %model.w(J,:) = wJ + alphas(epoch) * dwJ';
            %model.w(K,:) = wK - alphas(epoch) * dwK'; 
            %lr = (sqrt(1 - beta2_power) / (1 - beta1_power));
            
            
            model.w(J,:) = wJ + alphas(epoch) *   ( mhJ' ./ (sqrt(vhJ')+ 1e-8) );
            model.w(K,:) = wK - alphas(epoch) *   ( mhK' ./ (sqrt(vhK')+ 1e-8) );
            % update matrices
            if epsilons(epoch)>0, % epoch >= MatrixStart
                f1 = (2*dK/norm_factor)*2*(model.omega*DJ')*DJ;
                f2 = (2*dJ/norm_factor)*2*(model.omega*DK')*DK;
                ...
                f  = act * (f1 - f2 - regularization * f3);
                mf = beta1 * mf + (1. - beta1) * f; 
                vf = beta2 * vf + (1 -  beta2) * (f .^ 2);          
                
                mhf = mf / (1. - beta1_power);
                vhf = vf / (1. - beta2_power);
               
                
                model.omega = model.omega- epsilons(epoch) *  ((mhf ./ (sqrt(vhf) + 1e-8)));
